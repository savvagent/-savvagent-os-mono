...
export default class TinyUri {
  ...
  authority(authority = '') {
    if (authority !== '') {
      const auth = authority.match(this.authRegEx);
      this._authority = authority;
      if (auth) {
        authority = authority.replace(this.authRegEx, '');
        this.userInfo(auth[1]);
      }
      const port = authority.match(this.portRegEx);
      if(port) {
   ...
   */
  parse(uri) {
    const f = uri ? uri.match(this.uriRegEx) : [];
    const t = uri ? uri.match(this.urlTempQueryRegEx) : [];
    this.scheme(f[2]);
    this.authority(f[4]);
